...
> 在线体验：**[http://106.15.92.8](http://106.15.92.8)**

## 功能

- **自由绘制路线** — 在高德地图上按住鼠标拖动绘制任意闭合跑步路线
- **真实生理模拟** — 自动生成心率（三阶段热身→稳定→冲刺）、踏频、功率、海拔、步幅等数据
- **全参数可配** — 配速范围、时间范围、圈数、体重、心率区间、海拔变化、偏移量
...
- **精准定位来自客户端浏览器**：页面使用用户设备的 GPS / Wi-Fi 定位，不会定位到服务器位置。
- **线上必须使用 HTTPS**：除 `localhost` 外，浏览器通常只允许 HTTPS 页面调用精准定位。
- **高德地图坐标已校正**：浏览器返回 WGS-84 坐标，页面显示前会转换为 GCJ-02 以对齐高德地图。
- **定位失败需手动选择**：如果用户拒绝授权、设备无法定位或非 HTTPS，页面会提示手动拖动地图到目标位置，不会使用 IP 粗定位。
